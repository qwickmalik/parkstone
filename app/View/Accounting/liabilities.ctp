...
<div id="content">
    ...
    <div id="clearer"></div>
    <?php echo $this->Form->create('Transaction', array("url" => array('controller' => 'Accounting', 'action' => 'liabilities'), "inputDefaults" => array('div' => false,))); ?>
...
//            echo $this->Form->input('transaction_date', array('type' => 'date', 'value' => isset($ex['Transaction']['transaction_date']) ? date('d-m-Y',strtotime($ex['Transaction']['transaction_date'])) : date('d-m-Y'), 'dateFormat' => 'DMY', 'class' => 'form-control', 'div' => array('class' => 'form-inline')));
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <?php
                    echo $this->Form->input('headid', array('label' => 'Accounting Head', 'options' => $headids, 'value' => 5, 'disabled' => true));
            echo $this->Form->hidden('head_id', array('value' => 5));
            ?>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <?php
                    echo $this->Form->input('category_id', array('required', 'label' => 'Category*', 'options' => $categoryids, 'empty' => '--Please Select--', 'value' => isset($ex['Transaction']['category_id']) ? $ex['Transaction']['category_id'] : ''));
                    ?>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <?php
            echo $this->Form->input('amount', array('required', 'label' => 'Value/Amount*', 'value' => isset($ex['Transaction']['amount']) ? $ex['Transaction']['amount'] : '')); 
            ?>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <fieldset>
                        ...
                        echo $this->Form->input('effect', array(
                            ...
                            'hiddenField' => false, 
                            'legend' => false,
                            'before' => '<label>Effect on Liability</label><br />',
    ...
    <table class="table table-striped">
        ...
        foreach ($data as $each_item):
            ...
            <tr>
                ...
                <td align="right"><?php echo $this->Html->link(number_format($each_item['Transaction']['amount'], 2, '.', ','), '/Accounting/liabilities/' . $each_item['Transaction']['id']); ?></td>
                <td align="left"><?php 
                    if($each_item['Transaction']['effect']==='1'){
                        echo '<span style="color: red;">Incr.</span>';
                    }elseif ($each_item['Transaction']['effect']==='0'){
                        echo '<span style="color: green;">Decr.</span>';
                    }else{
                        echo '';
                    }
                ?></td>
                ...
                <td align="left"><?php echo $this->Html->link('Del', '/Accounting/delLiability/' . $each_item['Transaction']['id'], array('confirm' => 'Are you sure you want to delete?')); ?></td>
            </tr>
        <?php endforeach; ?>
