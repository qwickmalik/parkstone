                            ...
                            <tr>
                                ...
                                <td align="right" valign="top"><?php
                                    if (($each_item['Investment']['status'] == 'Rolled_over') || ($each_item['Investment']['status'] == 'Termination_Requested' && $each_item['Investment']['old_status'] == 'Rolled_over')) {
                                        if (isset($each_item['Investment']['rollover_amount'])) {
                                            $total_principal +=$each_item['Investment']['rollover_amount'];
                                            echo number_format($each_item['Investment']['rollover_amount'], 2);
                                        }
                                    } else {
                                        if (isset($each_item['Investment']['investment_amount'])) {
                                            $total_principal +=$each_item['Investment']['investment_amount'];
...
//                                        $id = $each_item['Investment']['id'];
                                        $interest_accrued = $each_item['Investment']['interest_accrued'];//$this->requestAction('/Investments/get_totalaccruedinterest/' . $id);
                                ...
                                <td align="right" valign="top"><?php
                                    if (($each_item['Investment']['status'] == 'Rolled_over') || ($each_item['Investment']['status'] == 'Termination_Requested' && $each_item['Investment']['old_status'] == 'Rolled_over')) {
                                        if (isset($each_item['Investment']['interest_accrued']) && isset($each_item['Investment']['rollover_amount'])) {
                                            $totals = $each_item['Investment']['interest_accrued'] + $each_item['Investment']['rollover_amount'];
                                            $total_pi += $totals;
...
//                                foreach ($total as $each_item):
                            ...
                            <?php
                            if (isset($total_principal)) {
                                echo 'GHc '.number_format($total_principal, 2);
                            }
                            ?></td>
                                <td align="right" valign="top" style="border-bottom: solid 1px #ffffff; background: #eaeaea; font-weight: bold;">&nbsp;</td>
                                <td align="right" valign="top" style="border-bottom: solid 1px #ffffff; background: #eaeaea; font-weight: bold;">&nbsp;</td>
                    ...
                    if (isset($total_interest)) {
                        echo 'GHc '.number_format($total_interest, 2);
                    }
                    ?></td>
                                <td align="right" valign="top" style="border-bottom: solid 1px #ffffff; background: #eaeaea; font-weight: bold;">
            <?php
            if (isset($total_pi)) {
                echo 'GHc '.number_format($total_pi, 2);
            }
            ?></td>
                            </tr>
    <?php
