...
<div id="content">
    ...
    <div id="clearer"></div>
    <?php echo $this->Form->create('Transaction', array("url" => array('controller' => 'Accounting', 'action' => 'income'), "inputDefaults" => array('div' => false,))); ?>
...
//            echo $this->Form->input('transaction_date', array('type' => 'date', 'value' => isset($ex['Transaction']['transaction_date']) ? date('d-m-Y',strtotime($ex['Transaction']['transaction_date'])) : date('d-m-Y'), 'dateFormat' => 'DMY', 'class' => 'form-control', 'div' => array('class' => 'form-inline')));
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <?php 
                    echo $this->Form->input('headid', array('label' => 'Accounting Head', 'options' => $headids, 'value' => 1, 'disabled' => true));
                    echo $this->Form->hidden('head_id', array('value' => 1));
        //             echo $this->Form->hidden('transaction_type', array('value' => 'Withdrawal'));
                ?>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <?php
                    echo $this->Form->input('category_id', array('required', 'label' => 'Income Category*', 'empty' => '--Please Select--', 'value' => isset($ex['Transaction']['category_id']) ? $ex['Transaction']['category_id'] : ''));
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    ...
                    echo $this->Form->hidden('effect', array('value' => 1)); //effect is always an increase
                    echo $this->Form->hidden('old_effect', array('value' => 1)); 
                    ?>
                </div>
            </div>
            <div class="row">
...
<div id="clearer"></div>
    ...
    <table class="table table-striped">
        <tr>
            ...
            <th style="border-bottom: solid 2px dodgerblue;" align="left"><b><?php echo $this->Paginator->sort('category_id', 'Income Category'); ?></b></th>
        ...
        foreach ($data as $each_item):
            ...
            <tr>
                ...
                <td align="right"><?php echo $this->Html->link(number_format($each_item['Transaction']['amount'], 2, '.', ','), '/Accounting/income/' . $each_item['Transaction']['id']); ?></td>
                ...
                <td align="left"><?php echo $this->Html->link('Del', '/Accounting/delIncome/' . $each_item['Transaction']['id'], array('confirm' => 'Are you sure you want to delete?')); ?></td>
            </tr>
        <?php endforeach; ?>
